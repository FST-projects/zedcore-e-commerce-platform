...
if (isset($_SESSION["admin"])) {
    ...
    if ($permissionValidation->num_rows  == 1) {
        ...
        if (isset($_GET["term"])) {
            ...
            $invoice_rs = Database::search("SELECT * FROM `admin` WHERE `fname` LIKE '%" . $term . "%' OR `lname` LIKE '%" . $term . "%' OR `admin_email` LIKE '%" . $term . "%' OR `role` LIKE '%" . $term . "%'");
            ...
            $result = Database::search("SELECT * FROM `admin` WHERE `fname` LIKE '%" . $term . "%' OR `lname` LIKE '%" . $term . "%' OR `admin_email` LIKE '%" . $term . "%' OR `role` LIKE '%" . $term . "%' ORDER BY `admin_id` DESC LIMIT " . $results_per_page . " OFFSET " . $page_results . "");
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        for ($x = 0; $x < $result_num; $x++) {
                            ...
                            $product_add_date = explode(" ", $total_product_data["added_date"]);
                        ...
                        ?>
                            ...
                            <tr>
                                <td style="min-width: 80px; pointer-events: none;">
                                    <center><?php echo $total_product_data['admin_id'] ?></center>
                                </td>
                                ...
                                <td>
                                    <?php
                                    if ($total_product_data["status"] == 1) {
                                    ?>
                                        <button id="adminstatus<?php echo $total_product_data['admin_id'] ?>" class="ui teal button" style="width: 120px;" onclick="changeAdminStatus(<?php echo $total_product_data['admin_id'] ?>);">Activated</button>
                                    <?php
                                    } else if ($total_product_data["status"] == 2) {
                                    ?>
                                        <button id="adminstatus<?php echo $total_product_data['admin_id'] ?>" class="ui red button" style="width: 120px;" onclick="changeAdminStatus(<?php echo $total_product_data['admin_id'] ?>);">Blocked</button>
                                    <?php
                                    }
                                    ?>
                                </td>
